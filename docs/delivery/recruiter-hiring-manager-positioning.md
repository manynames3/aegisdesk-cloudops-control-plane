...
I built a local-first CloudOps AI control plane that separates natural language interaction from authorization and execution. The gateway derives identity from signed demo tokens, inspects requests, redacts sensitive data, evaluates OPA/Rego policies, routes between local and optional cloud models, gates MCP-style tool calls, emits audit events and traces, and exposes operational state in an admin dashboard.
