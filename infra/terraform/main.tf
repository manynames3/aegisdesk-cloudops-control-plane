  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = var.incident_cloudwatch_log_group_arn
      },
      {
...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      AEGISDESK_INCIDENT_CONTEXT_ADAPTER          = var.incident_context_adapter
      AEGISDESK_CLOUDWATCH_LOG_GROUP              = var.incident_cloudwatch_log_group
      AEGISDESK_CLOUDWATCH_LOGS_REGION            = var.aws_region
      AEGISDESK_CLOUDWATCH_QUERY_LOOKBACK_MINUTES = tostring(var.incident_query_lookback_minutes)
      AEGISDESK_CLOUDWATCH_QUERY_LIMIT            = tostring(var.incident_query_limit)
      AEGISDESK_AUDIT_RETENTION_DAYS              = tostring(var.audit_retention_days)
      AEGISDESK_AUDIT_EXPORT_MAX_EVENTS           = tostring(var.audit_export_max_events)
